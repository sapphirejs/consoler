...
class Consoler {
  ...
  constructor(route, cli) {
    this._route = this._parseCommand(route)
    this._cli = this._parseCommand(cli)

    this._types = ['string', 'number', 'array']
    this._command = {
      command: null,
      argument: {},
      option: {}
  ...
  _parseOptions(routeOptions, cliOptions) {
    ...
    for (let option of Object.keys(routeOptions)) {
      ...
      const cliOptValue = this._cast(cliOptions[option])
      const { name, type, _default } = this._parsePlaceholders(routeOptValue)
