...
class Consoler {
  ...
  _parseArguments(routeArguments, cliArguments) {
    ...
    routeArguments.forEach((argument, index) => {
      ...
      // Optional or unamed arguments.
        ...
        // The cli argument must also exist to take
        // it into consideration.
        if (/\[(.+)\]/.test(argument) && cliArg) {
          const matches = argument.match(/\[(.+)\]/)
          args[matches[1]] = cliArg
          argument = matches[1]
        }
      }
    })
  ...
  _parseOptions(routeOptions, cliOptions) {
    ...
    for (let option of Object.keys(routeOptions)) {
      ...
      const cliOptValue = this._cast(cliOptions[option] || cliOptions[alias] || _default)

      // No option actually received from cli.
      if (!cliOptValue) continue
